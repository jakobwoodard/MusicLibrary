...
class MusicLibrary:
    ...
    def fit_highest_value_songs(self, playlist_time):
        available_time = playlist_time * 60  # Convert minutes to seconds
        num_songs = len(self.songs)

        # Initialize DP table
        dp = [[0] * (available_time + 1) for _ in range(num_songs + 1)]

        # Build the DP table
        for i in range(1, num_songs + 1):
            for w in range(available_time + 1):
                if self.songs[i - 1].duration <= w:
                    dp[i][w] = max(
                        self.songs[i - 1].score
                        + dp[i - 1][w - self.songs[i - 1].duration],
                        dp[i - 1][w],
                    )
                else:
                    dp[i][w] = dp[i - 1][w]
        # Trace back to find the songs to include in the playlist
        w = available_time
        playlist = []
        for i in range(num_songs, 0, -1):
            if dp[i][w] != dp[i - 1][w]:
                playlist.append(self.songs[i - 1])
                w -= self.songs[i - 1].duration

        print(
            f"\nPlaylist maximizing score within {playlist_time} minutes ({available_time} seconds):"
        )
        self.display_songs(playlist)

        total_score = sum(song.score for song in playlist)
        total_duration = sum(song.duration for song in playlist)
        minutes = total_duration // 60
        seconds = total_duration % 60
        print(
            f"\nTotal playlist length: {minutes} minutes and {seconds} seconds with total score: {total_score}"
        )

    def compute_distance(self, song1, song2):
        ...
        score_diff = abs(song1.score - song2.score)
        return math.sqrt(
            danceability_diff**2
            + energy_diff**2
            + valence_diff**2
    ...
    def k_nearest_neighbors(self, target_song_title, k=5):
        target_song = self.search_in_hashtable(target_song_title)
        if not target_song:
            print(f"Song titled '{target_song_title}' not found in the library.")
            return []
        distances = []
        for song in self.songs:
            if song.title != target_song.title:
                distance = self.compute_distance(target_song, song)
                distances.append((song, distance))

        distances.sort(key=lambda x: x[1])
        nearest_neighbors = [song for song, _ in distances[:k]]

        return nearest_neighbors

    def display_knn(self, target_song_title, k=5):
        nearest_neighbors = self.k_nearest_neighbors(target_song_title, k)
        if nearest_neighbors:
            print(f"\n{k} Nearest Neighbors to '{target_song_title}':")
            self.display_songs(nearest_neighbors)
        else:
            print(
                f"No nearest neighbors found or song '{target_song_title}' is not in the library."
            )
