...
class MusicLibrary:
    ...
    def add_song(self, song):
        ...
        self.genre_bst.insert(song.__dict__)
        self.avl_root = self.avl_tree.insert(self.avl_root, song.__dict__)
        ...
        for songs in artist_to_songs.values():
            for i in range(len(songs)):
                for j in range(i + 1, len(songs)):
                    ...
                    self.graph.add_edge(songs[j], songs[i])
    ...
    def display_songs_by_genre(self, genre):
        found_songs = self.genre_bst.search(genre)
        if found_songs:
            print(f"\nFound Songs in Genre '{genre}' (including subgenres):")
            headers = f"{'Title':25} {'Artist':20} {'Score':5} {'Genre':10} {'Danceability':12} {'Energy':10} {'Valence':10} {'Duration':8}"
            print(headers)
            print("-" * len(headers))
            for song_dict in found_songs:
                song = Song(**song_dict)
                print(song)
        else:
            print(f"\nNo songs found in Genre '{genre}' or its subgenres")

    def display_songs_by_avl(self):
        print("\nSongs in AVL Tree (Inorder Traversal):")
        for song in self.avl_tree.inorder_traversal(self.avl_root):
            print(Song(**song))

    def search_songs_in_avl_by_genre(self, genre):
        found_songs = self.avl_tree.search_by_genre(self.avl_root, genre)
        if found_songs:
            print(f"\nFound Songs in Genre '{genre}' (AVL Tree):")
            headers = f"{'Title':25} {'Artist':20} {'Score':5} {'Genre':10} {'Danceability':12} {'Energy':10} {'Valence':10} {'Duration':8}"
            print(headers)
            print("-" * len(headers))
            for song_dict in found_songs:
            ...
            print(f"\nNo songs found in Genre '{genre}' (AVL Tree)")
